...
export default class Treeview extends mixins(
  UpdatableItemMixin,
  FilterAwareMixin
) {
  ...
  mounted() {
    this.$store.subscribe((mutation, store) => {
      if (
        mutation.type === "novelData/novelItemAdded" &&
        this.isAboutChildItem(mutation)
      ) {
        this.selectItemIds({
          view: mutation.payload.view,
          itemIds: [mutation.payload.novelItem.id],
        });
        this.addTagsIfNeeded(mutation.payload.novelItem as ChildModel);
      }
      if (
        mutation.type === "novelData/novelItemDeleted" &&
        this.isAboutChildItem(mutation)
      ) {
      ...
      if (
        mutation.type === "novelData/novelItemsLoaded" &&
        !this._selectedItemIds[mutation.payload.view]?.length
      ) {
        const firstParentWithChildren = this.getFilteredItems(
          mutation.payload.novelItems
        ).find((parent) => parent.children.length > 0);
        if (firstParentWithChildren) {
          this.selectItemIds({
            view: mutation.payload.view,
            itemIds: [firstParentWithChildren.children[0].id],
          });
        }
      }
    });
  ...
  isOpen(parent: ParentModel) {
    return Object.keys(this.currentToggleState).includes(`${parent.id}`)
      ? this.currentToggleState[parent.id]
      : true;
  }
  ...
  handleCtrl(selected: BaseModel): number[] {
    const index = this.selectedItems.findIndex(
      (selectedItem) => selectedItem === selected.id
    );
    if (index >= 0) {
  ...
  handleShift(selected: BaseModel): number[] {
    ...
    const newSelectedItems = this.childItems.slice(
      Math.min(start, end),
